#!/bin/bash
# Build and optionally push the thath/kilocode image.
#
# Usage:
...
Without --push, builds a local image tagged kilocode:latest.
EOF
            exit 0 ;;
...
# ---------------------------------------------------------------------------
    ...
    TAGS=(-t "kilocode:latest")
    [[ -n "$TAG" && "$TAG" != "latest" ]] && TAGS+=(-t "kilocode:$TAG")

    echo "Building kilocode:latest ($(uname -m))..."
    docker build "${TAGS[@]}" "$SCRIPT_DIR"
    echo "Done. Image tagged kilocode:latest"
fi
